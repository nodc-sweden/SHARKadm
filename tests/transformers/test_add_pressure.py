...
@pytest.mark.parametrize(
    ...
    (
        ("DEF456", 57.1, 11.0, -0.1),  # erroneous depth
        ("GHJ789", 56.5, 12.0, -100),  # erroneous depth
    ),
)
@patch("sharkadm.config.get_all_data_types", return_value=[])
def test_validate_add_pressure_erroneous_data(
    mocked_data_types,
    ...
    # should still not exist if the depths are not valid
    assert "Derived pressure" not in given_data_holder.data.columns, (
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("GHJ789", 56.5, 12.0, 100, True),  # ok data
    ),
)
@patch("sharkadm.config.get_all_data_types", return_value=[])
def test_validate_add_pressure(
    ...
    # The calculated pressure will be a float
    if expected_success:
