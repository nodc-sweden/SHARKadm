...
class EpibenthosArchiveDataHolder(ArchiveDataHolder):
    ...
    def _load_data(self) -> None:
        ...
        d_source.remove_columns('SMPNO')  # Thera are soemtimes duplicate columns mapping to sample_id. Note sure if this breaks anything else
...
class EpibenthosMartransArchiveDataHolder(ArchiveDataHolder):
    ...
    def _load_data(self) -> None:
        ...
        d_source = data_source.XmlMartransDataFile(path=data_file_path, data_type=self.delivery_note.data_type)
        d_source.map_header(self.import_matrix_mapper)
