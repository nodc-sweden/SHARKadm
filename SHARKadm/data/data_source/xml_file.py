...
class XmlMartransDataFile(DataFile):
    ...
    def _load_file(self) -> None:
        ...
        for undersokning in root.findall('undersokning'):
            all_tags.update([f'undersokning.{el.tag}' for el in undersokning if not el.text.strip()])
            undersokning_data = dict((f'undersokning.{el.tag}', el.text.strip()) for el in undersokning if el.text.strip())

            for lokal in undersokning.findall('lokal'):
                all_tags.update([f'lokal.{el.tag}' for el in lokal if not el.text.strip()])
                lokal_data = dict((f'lokal.{el.tag}', el.text.strip()) for el in lokal if el.text.strip())

                for tillfalle in lokal.findall('tillfalle'):
                    all_tags.update([f'tillfalle.{el.tag}' for el in tillfalle if not el.text.strip()])
                    tillfalle_data = dict((f'tillfalle.{el.tag}', el.text.strip()) for el in tillfalle if el.text.strip())
                    # print([el.tag for el in tillfalle if not el.text.strip()])

                    for hydrografi in tillfalle.findall('hydrografi'):
                        all_tags.update([f'hydrografi.{el.tag}' for el in hydrografi if not el.text.strip()])
                        hydrografi_data = dict(
                            (f'hydrografi.{el.tag}', el.text.strip()) for el in hydrografi if el.text.strip())

                    for transekt in tillfalle.findall('transekt'):
                        all_tags.update([f'transekt.{el.tag}' for el in transekt if not el.text.strip()])
                        transekt_data = dict((f'transekt.{el.tag}', el.text.strip()) for el in transekt if el.text.strip())

                        for avsnitt in transekt.findall('avsnitt'):
                            all_tags.update([f'avsnitt.{el.tag}' for el in avsnitt if not el.text.strip()])
                            avsnitt_data = dict((f'avsnitt.{el.tag}', el.text.strip()) for el in avsnitt if el.text.strip())
                            # print([f'avsnitt.{el.tag}' for el in avsnitt if not el.text.strip()])

                            for avsnittart in avsnitt.findall('avsnittart'):
                                all_tags.update([f'avsnittart.{el.tag}' for el in avsnittart if not el.text.strip()])
                                avsnittart_data = dict(
                                    (f'avsnittart.{el.tag}', el.text.strip()) for el in avsnittart if el.text.strip())
                                # print([f'avsnitt.{el.tag}' for el in avsnitt if not el.text.strip()])
                                data = dict()
                            ...
                            for avsnittsubstrat in avsnitt.findall('avsnittsubstrat'):
                                all_tags.update(
                                    [f'avsnittsubstrat.{el.tag}' for el in avsnittsubstrat if not el.text.strip()])
                                avsnittsubstrat_data = dict(
                                    (f'avsnittsubstrat.{el.tag}', el.text.strip()) for el in avsnittsubstrat if
                                    el.text.strip())
                                # print([f'avsnitt.{el.tag}' for el in avsnitt if not el.text.strip()])
                                data = dict()
                                data.update(meta_data)
                        ...
                        for transekttaxaminmax in transekt.findall('transekttaxaminmax'):
                            all_tags.update(
                                [f'transekttaxaminmax.{el.tag}' for el in transekttaxaminmax if not el.text.strip()])
                            transekttaxaminmax_data = dict(
                                (f'transekttaxaminmax.{el.tag}', el.text.strip()) for el in transekttaxaminmax if
                                el.text.strip())
                            # print([f'transekttaxaminmax.{el.tag}' for el in transekttaxaminmax if not el.text.strip()])
                            data = dict()
                            data.update(meta_data)
                            data.update(undersokning_data)
                            data.update(lokal_data)
                        ...
                        for prov in transekt.findall('prov'):
                            all_tags.update([f'prov.{el.tag}' for el in prov if not el.text.strip()])
                            prov_data = dict((f'prov.{el.tag}', el.text.strip()) for el in prov if el.text.strip())
                            # print([f'prov.{el.tag}' for el in prov if not el.text.strip()])

                            for provsubstrat in prov.findall('provsubstrat'):
                                all_tags.update([f'provsubstrat.{el.tag}' for el in provsubstrat if not el.text.strip()])
                                provsubstrat_data = dict(
                                    (f'provsubstrat.{el.tag}', el.text.strip()) for el in provsubstrat if el.text.strip())
                                # print([f'avsnitt.{el.tag}' for el in avsnitt if not el.text.strip()])
                                data = dict()
                                data.update(meta_data)
                                data.update(undersokning_data)
                            ...
                            for provart in prov.findall('provart'):
                                all_tags.update([f'provart.{el.tag}' for el in provart if not el.text.strip()])
                                provart_data = dict(
                                    (f'provart.{el.tag}', el.text.strip()) for el in provart if el.text.strip())
                                # print([f'avsnitt.{el.tag}' for el in avsnitt if not el.text.strip()])
                                data = dict()
                                data.update(meta_data)
                                data.update(undersokning_data)
    ...
    #     cols = ['avsnittart.taxon_namn', 'provart.taxon_namn', 'taxon.taxon_namn', 'transekttaxaminmax.taxon_namn']
    ...
    #     cols = ['transekttaxaminmax.transekttaxaminmax_taxonID', 'avsnittart.taxonID', 'provart.provart_taxonID', 'provart.taxonID']
    #     cols = [col for col in cols if col in self._data.columns]
    #     self._data['dyntaxa_id'] = self._data[cols].apply(''.join, axis=1)
    #     self._data.drop(cols, axis='columns', inplace=True)
    #
    # def _fix_variable_comment(self):
    #     cols = ['provart.provart_kommentar', 'transekttaxaminmax.transekttaxaminmax_kommentar', 'avsnitt.avsnittart_kommentar', 'bitmarken.bitmarken_beskrivning', 'nyrekrytering.nyrekrytering_beskrivning', 'avsnittart.kommentar', 'provart.kommentar', 'avsnittart.nyrekrytering_nyrekrytering', 'avsnittart.bitmarken_bitmarken', 'transekttaxaminmax.kommentar	segmenttaxonkommentar', 'kommentar', 'COMNT_VAR']
    #     cols = [col for col in cols if col in self._data.columns]
    #     self._data['variable_comment'] = self._data[cols].apply(''.join, axis=1)
    #     self._data.drop(cols, axis='columns', inplace=True)
    #
    # def _fix_cover(self):
    #     cols = ['provartstorlek.provartstorlek_abundans', 'provart.provart_abundans', 'avsnittart.avsnittart_antal', 'avsnittart.antal', 'provart.abundans']
    #     cols = [col for col in cols if col in self._data.columns]
    #     self._data['variable.COPY_VARIABLE.Cover.%'] = self._data[cols].apply(''.join, axis=1)
    #     self._data.drop(cols, axis='columns', inplace=True)
    #
    # def _fix_taxon_photo(self):
    #     cols = ['avsnittart.foto', 'provart.provart_foto', 'provart.foto', 'avsnitt.foto']
    #     cols = [col for col in cols if col in self._data.columns]
