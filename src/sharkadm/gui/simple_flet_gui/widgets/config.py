...
class Configs(ft.UserControl):
    ...
    def _create_controls(self):
        ...
        self._add_button = ft.ElevatedButton(
            "Spara nuvarande configuration", on_click=self._try_save_current_config
        )
        self._add_name = ft.TextField(
            hint_text="Namn på ny configfil", suffix_text=".yaml"
        )
    ...
    def _create_ask_overwrite_dialog(self) -> None:
        self._ask_overwrite_dialog = ft.AlertDialog(
            ...
            content=ft.Text(
                "Det finns redan en konfiguration med det här namnet. "
                "Vill du skiva över den befintliga?"
            ),
            actions=[
    ...
    def _get_layout(self):
        ...
        add_row = ft.Row([self._add_name, self._add_button])
    ...
    def _list_config_files(self):
        ...
        for path in sorted(utils.get_config_directory().iterdir()):
            if path.suffix != ".yaml":
                continue
            self._config_list.controls.append(
                ft.ElevatedButton(
                    path.stem, on_click=lambda x, p=path: self._load_config(x, p)
                )
            )
    ...
    def _save_current_config(self, overwrite: bool = False) -> None:
        ...
        gui_event.post_event("save_config", dict(file_path=path, overwrite=overwrite))
