...
class ValidateWaves(Validator):
    ...
    def _validate(self, data_holder: DataHolderProtocol) -> None:
        ...
        unique_rows = (
            ...
            .group_by(["visit_date", "reported_station_name", "wave_observation_code"])
            .agg(pl.col("row_number").alias("row_numbers"))
        )
        unique_rows = unique_rows.with_columns(
            pl.when(pl.col("wave_observation_code").is_in(valid_values))
            .then(pl.lit("Wave observation code is ok"))
            .when(
                pl.col("wave_observation_code").is_null()
                | (pl.col("wave_observation_code").str.strip_chars() == "")
            )
            .then(
                pl.format(
                    "{} on {}: Missing wave observation code: {}",
            ...
            .otherwise(
                pl.format(
                    "{} on {}: Wave observation code has unexpected value: {}",
                    pl.col("reported_station_name"),
                    pl.col("visit_date"),
                    pl.col("wave_observation_code"),
                )
        ...
        if (
            unique_rows.filter(pl.col("message") != "Wave observation code is ok").height
            == 0
        ):
            self._log_success("All wave observation codes are ok")
        else:
            for (msg,), df in unique_rows.filter(
                pl.col("message") != "Wave observation code is ok"
            ).group_by("message"):
