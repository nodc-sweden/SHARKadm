    ...
    def _validate(self, data_holder: DataHolderProtocol) -> None:
        for col in MissingTime.source_cols:
            if col not in data_holder.data.columns:
                adm_logger.log_validation(f'Missing column "{col}"', level=adm_logger.ERROR)
                continue
            ...
            adm_logger.log_validation(f'Missing {col} if {nr_missing} rows. Att rows: {rows_str}', level=adm_logger.ERROR)
