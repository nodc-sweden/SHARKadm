...
class ValidateDuplicatedRows(Validator):
    ...
    def get_validator_description() -> str:
        return f"Check for duplicated rows"

    def _validate(self, data_holder: PolarsDataHolder) -> None:
        if self.include_columns:
            columns = [col for col in self.include_columns if
                       col in data_holder.data.columns]
        else:
            columns = [col for col in data_holder.data.columns if
                       col not in self.exclude_columns]

        data = data_holder.data.filter(
            data_holder.data[columns].is_duplicated()
        )
        log_cols = [col for col in self.log_columns
