...
class PolarsBvolScientificNameInDyntaxa(Validator):
    ...
    def get_validator_description() -> str:
        return (
            ...
            f"{PolarsTranslatedDyntaxaScientificNameInDyntaxa.col_to_check} are in "
            f"dyntaxa (taxon.csv)"
        )
...
class ValidateBvolSizeClass(Validator):
    ...
    def _validate(self, data_holder: PolarsDataHolder) -> None:
        if self.scientific_name_col not in data_holder.data.columns:
            adm_logger.log_validation(f"Could not validate bvol size class. "
    ...
    def _map_and_validate(self, data_holder: PolarsDataHolder):
        ...
        for (name, size_class), dd in df.group_by(self.scientific_name_col,
                                                    self.size_class_col):
            self._log_fail(
                    f"Invalid bvol size class {size_class} for {name} "
                    f"({len(dd)} places)",
                    level=adm_logger.WARNING,
            )
