    ...
    def validate(self, data_holder: DataHolderProtocol) -> None:
        if data_holder.data_type_internal not in config.get_valid_data_types(valid=self.valid_data_types,
            ...
            adm_logger.log_workflow(
                f'Invalid data_type {data_holder.data_type_internal} for validator {self.__class__.__name__}', level=adm_logger.DEBUG)
            return
        ...
        adm_logger.log_workflow(f'Applying validator: {self.__class__.__name__}', item=self.get_validator_description(), level=adm_logger.DEBUG)
        t0 = time.time()
