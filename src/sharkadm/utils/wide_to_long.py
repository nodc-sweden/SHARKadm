...
class WideToLong:
    ...
    def __init__(
        self,
        df: pd.DataFrame,
        prefix: str = "COPY_VARIABLE",
        var_col_name: str = "parameter",
    ...
    def _wide_to_long(self) -> None:
        self._df = self._df.melt(
            id_vars=[col for col in self._df.columns if not col.startswith(self._prefix)],
            var_name=self._var_col_name,
        )

    def _cleanup(self) -> None:
        self._df[self._unit_col_name] = self._df[self._var_col_name].apply(self._fix_unit)
        self._df[self._var_col_name] = self._df[self._var_col_name].apply(
            self._fix_parameter
        )
