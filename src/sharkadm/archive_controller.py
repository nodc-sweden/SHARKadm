...
class ArchiveController:
    ...
    def filter_columns(
        ...
        exclude_columns = []
        if include:
            include_columns = matching_strings.get_matching_strings(
                self.data.columns, include
            )
        if exclude_columns:
            exclude_columns = matching_strings.get_matching_strings(
                self.data.columns, exclude
            )
        columns = []
    ...
    def create_delivery_note_summary(self):
        ...
        for path in self.archive_list:
            ...
            if not delivery_note_path.exists():
                ...
                try:
                    delivery_note = DeliveryNote.from_txt_file(
                        delivery_note_path, include_all_column=True
                    )
        ...
        cols_to_use = dict(
            (key, cols_to_use[key])
            for key in sorted(cols_to_use, key=lambda x: x.lower())
        )

        # other_columns = sorted(
        #     [col for col in info_set if col.lower() not in first_cols]
        # )
        # cols_to_use = [
        #     'archive_directory_name',
        #     'delivery_note_missing',
        ...
        self._save_path = (
            utils.get_export_directory() / f"delivery_note_summary_{self._data_type}.txt"
        )
        return self
    ...
    def create_raw_delivery_note_summary(self):
                ...
                try:
                    ...
                    with open(delivery_note_path, encoding=encoding) as fid:
                        ...
                        for line in fid:
                            ...
                            if key.upper() == "FORMAT":
                                ...
                                if len(parts) == 1:
                                    msg = (
                                        f"Can not find any import_matrix_key "
                                        f"(data_format) in delivery_note: {path}"
                                    )
                    ...
                    with open(delivery_note_path, encoding=encoding) as fid:
                        ...
                        if "depth" in all_data.lower() or "djup" in all_data.lower():
                            with open(
                                r"C:\mw\DV_strul\depth_in_delivery_note.txt", "a"
                            ) as fid:
                                fid.write(
                                    f"{delivery_note_path.parent.parent.name}\t"
                                    f"{all_data}\n"
                                )
        ...
        # column_mapping = {}
        # first_cols = [
        #     'archive_directory_name', 'delivery_note_missing', 'data_format_missing'
        # ]
        # collection = {}
        # cols_to_use = dict((item, item) for item in first_cols)
        # for col in info_set:
    ...
    def create_shark_metadata_summary(self):
        ...
        self._create_dataframe(info, cols_to_use)
        self._save_path = (
            utils.get_export_directory() / f"shark_metadata_summary_{self._data_type}.txt"
        )
        return self
    ...
    def create_column_summary(self, *columns: str, all_columns=False):
        ...
        for path in self.archive_list:
            ...
            try:
                ...
                sharkadm_exceptions.ArchiveDataError,
                sharkadm_exceptions.DeliveryNoteError,
            ):
                ...
                # try:
                #     delivery_note = DeliveryNote.from_txt_file(
                #         processed_folder / 'delivery_note.txt'
                #     )
        ...
        nr = 4
        string = "-".join(
            [col for col in self.data.columns if col not in ["archive", "format"]][:4]
        )
        if len(self.data.columns) > nr:
            string = string + "___"
        self._save_path = (
            utils.get_export_directory()
            / f"column_summary_{self._data_type}_{string}.txt"
        )
        return self
    ...
    def create_files_list(self):
        ...
        self._save_path = (
            utils.get_export_directory() / f"file_list_{self._data_type}.txt"
        )
        return self
    ...
    def _create_dataframe(self, info: list[dict], columns: list | dict):
        ...
        temp_list_columns_df = [
            item for item in self._df.columns if item.lower().startswith("kom")
        ]
