...
class ImportMatrixMapper:
    ...
    def __repr__(self) -> str:
        return (
            f'{self.__class__.__name__} with data type "{self._data_type}" '
            f'and column "{self._import_column}"'
        )
    ...
    def get_internal_name(self, external_par: str) -> str:
        ...
        if not self._data.get(external_par):
            adm_logger.log_workflow(
                f"Could not map parameter {external_par} using mapping column "
...
class ImportMatrixConfig:
    ...
    def __repr__(self) -> str:
        return (
            f'{self.__class__.__name__} with data type "{self._data_type}": {self._path}'
        )
    ...
    def get(self, import_column: str, external_par: str) -> str:
        """Returns the internal parameter name for the given institute and external
        parameter name"""
