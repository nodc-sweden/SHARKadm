...
class IfcbVisualizationCruises:
    ...
    def __init__(self, data: pd.DataFrame, directory: str | pathlib.Path, append: bool = True, **kwargs):
        self._data = data
        self._directory = pathlib.Path(directory)
        self._append = append
    ...
    def _create_cruises_file(self):
        ...
        for cruise, df in self._data.groupby(cruise_col):
            ...
            route_list = []
            for (lat, lon), dtime_df in df.sort_values('datetime').groupby(['sample_latitude_dd', 'sample_longitude_dd']):
    ...
    def _append_cruise_file(self, data: dict) -> None:
        ...
        old_cruise_mapper = {f"{item['startTime']}_{item['stopTime']}": item for item in old_data['cruises']}
        new_cruise_mapper = {f"{item['startTime']}_{item['stopTime']}": item for item in data['cruises']}
...
def create_ifcb_visualization_files(data: pd.DataFrame, directory: str | pathlib.Path, append: bool = True):
