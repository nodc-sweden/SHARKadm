...
class SpeciesTranslationTxt(FileExporter):
    ...
    def get_exporter_description() -> str:
        return (
            "Creates a txt file with all translations of scientific_name to dyntaxa, "
    ...
    def _export(self, data_holder: DataHolderProtocol) -> None:
        if not self.export_file_name:
            self._export_file_name = (
                f"species_translation_{data_holder.dataset_name}.txt"
            )
        columns = [col for col in self.columns if col in data_holder.data]
        df = (
            data_holder.data[data_holder.data["reported_scientific_name"] != ""]
            .drop_duplicates(columns)
            .sort_values(["reported_scientific_name"])
        )
