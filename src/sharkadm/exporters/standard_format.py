...
class StandardFormat(Exporter):
    ...
    def __init__(self,
        ...
        self._initial_rows = [r'// FORMAT=PROFILE',
    ...
    def _get_data(self,
        ...
        # d_data.insert(0, column='SECOND', value=data['datetime'].apply(lambda x: str(x.second).zfill(2)))
        # d_data.insert(0, column='MINUTE', value=data['datetime'].apply(lambda x: str(x.minute).zfill(2)))
        # d_data.insert(0, column='HOUR', value=data['datetime'].apply(lambda x: str(x.hour).zfill(2)))
        # d_data.insert(0, column='DAY', value=data['datetime'].apply(lambda x: str(x.day).zfill(2)))
        # d_data.insert(0, column='MONTH', value=data['datetime'].apply(lambda x: str(x.month).zfill(2)))
        # d_data.insert(0, column='YEAR', value=data['datetime'].apply(lambda x: str(x.year)))
    ...
    def _get_reordered_columns(columns: list[str]) -> list[str]:
        ...
        new_order = pres_cols + depth_cols + [col for col in columns if col not in exclude_cols]
        return new_order
