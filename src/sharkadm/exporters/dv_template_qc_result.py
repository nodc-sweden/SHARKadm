...
class ExportDvTemplateWithQcResult(PolarsFileExporter):
    ...
    def _export(self, data_holder: PolarsDataHolder) -> None:
        ...
        indata.save(self.export_file_path)
        print("TOTAL_QC-kolumner har lagts till och fyllts i Excel-filen.")


class LimsExportDvTemplateWithQcResult(PolarsFileExporter):
    def __init__(self, lims_template_file, **kwargs):
        self._lims_template_file = pathlib.Path(lims_template_file)
        super().__init__(**kwargs)
        if not kwargs.get("export_file_name"):
            self._export_file_name = f"{self._lims_template_file.stem}_qc_result.xlsx"

    @staticmethod
    def get_exporter_description() -> str:
        return "Export qc result from LIMS as DV template data"

    def _export(self, data_holder: PolarsDataHolder) -> None:
        df = pl.read_csv(
            self._lims_template_file,
            separator="\t",
            encoding="cp1252",
            infer_schema_length=None,
        )

        df.write_excel(f"{self._lims_template_file.stem}.xlsx")
        df = openpyxl.load_workbook(f"{self._lims_template_file.stem}.xlsx")
        tab = df["Sheet1"]
        ...
        for parameter in unique_parameters:
            ...
            tab[f"{total_qc_column}1"] = f"TOTAL_QC_{parameter}"
