...
class PolarsExporter(ABC, Operator):
    ...
    def export(self, data_holder: PolarsDataHolder) -> OperatorInfo | Any:
        ...
        if data is None:
            data = get_single_operators_info(operator=self)
        elif isinstance(data, OperatorInfo):
            data.operator = self
        return data
...
class PolarsFileExporter(PolarsExporter, ABC):
    ...
    def export(self, data_holder: PolarsDataHolder) -> OperatorsInfo:
        info = super().export(data_holder=data_holder)
        self.open_file()
