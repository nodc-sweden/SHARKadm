...
class SharkadmLoggerExporter(ABC):
    ...
    def _set_save_path(self, suffix):
        ...
        file_name = (
            self.kwargs.get("export_file_name")
            or self.kwargs.get("file_name")
            or self._get_default_file_name()
        )
        export_directory = self.kwargs.get("export_directory") or self.kwargs.get(
            "directory"
        )
        if file_path:
    ...
    def _open_directory(self):
        if not self.kwargs.get(
            "open_directory", self.kwargs.get("open_export_directory")
        ):
            return
        if not self.file_path:
            logger.info(
                f"open_directory is not implemented "
    ...
    def _open_file(self):
        if not self.kwargs.get(
            "open_report",
            self.kwargs.get("open_file", self.kwargs.get("open_export_file")),
        ):
            return
        if not self.file_path:
            logger.info(
                f"open_file is not implemented for exporter {self.__class__.__name__}"
            )
            return
