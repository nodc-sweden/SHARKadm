...
class ZipArchiveDataHolder(DataHolder, ABC):
    ...
    def _unzip_archive(self):
        self._unzipped_archive_directory = utils.unzip_file(self._zip_archive_path, utils.get_temp_directory('zip'), delete_old=True)
    ...
    def _load_data(self) -> None:
        if not self.shark_data_path.exists():
            logger.error(f'Could not find any data file in delivery: {self.shark_data_path}')
            return
