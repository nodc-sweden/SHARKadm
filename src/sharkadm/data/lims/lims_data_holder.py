...
from sharkadm.data.data_source import CsvRowFormatDataFilePolars, TxtColumnFormatDataFile, \
    DataFile
...
class LimsDataHolder(PandasDataHolder):
    ...
    def _load_data(self) -> None:
        d_source = TxtColumnFormatDataFile(path=self.data_file_path, data_type=self.data_type)
...
class PolarsLimsDataHolder(PolarsDataHolder):
    ...
    def _load_data(self) -> None:
        data_source = CsvRowFormatDataFilePolars(path=self.data_file_path, data_type=self.data_type)
        if self._header_mapper:
