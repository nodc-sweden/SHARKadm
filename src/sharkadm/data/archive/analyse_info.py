...
class AnalyseInfo:
    ...
    def get_uncertainty(
        self, par: str, date: datetime.date, value: float | str | None = None
    ) -> str:
        ...
        with open(path, encoding=encoding) as fid:
            ...
            for r, line in enumerate(fid):
                ...
                if len(split_line) != len(header):
                    adm_logger.log_workflow(
                        f"No or insufficient data in analyse_info for parameter: "
                        f"{split_line[0]}",
                ...
                for item in ["VALIDFR", "VALIDTO"]:
                    if not line_dict.get(item):
                        adm_logger.log_workflow(
                            f"Missing {item} in analyse_info", level=adm_logger.WARNING
                        )
                        line_dict[item] = None
                        continue
    ...
    @classmethod
    def from_lims_txt_file(
        cls, path: str | pathlib.Path, mapper: Mapper, encoding: str = "cp1252"
    ) -> "AnalyseInfo":
        path = pathlib.Path(path)
        if path.suffix != ".txt":
            msg = f"File is not a valid analyse_info text file: {path}"
    ...
    def from_dv_template(
        ...
        if not sheet_name:
            adm_logger.log_workflow(
                f"Could not find analyse_info sheet in file: {path}",
                level=adm_logger.WARNING,
            )
            return
