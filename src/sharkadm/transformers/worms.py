...
class AddWormsScientificName(Transformer):
    ...
    @staticmethod
    def get_transformer_description() -> str:
        return f'Adds {AddWormsScientificName.col_to_set} translated from nodc_worms. Source column is {AddWormsScientificName.source_col}'
    ...
    def _transform(self, data_holder: DataHolderProtocol) -> None:
        ...
        for name, df in data_holder.data.groupby(self.source_col):
            ...
            if new_name:
                adm_logger.log_transformation(f'Translate worms: {name} -> {new_name} ({len(df)} places)')
            else:
                new_name = name
