...
class AddLmqnt(Transformer):
    ...
    def _clean_lmqnt_str(self, lmqnt_str: str) -> str:
        if any(char.isdigit() for char in lmqnt_str):
            ...
            lmqnt_str = re.sub(r'[a-zA-Z]',
                                lambda match: match.group(0).lower() if match.group(0).lower() != 'c' else 'C',
                                lmqnt_str)
    ...
    def _parse_lmqnt(self, lmqnt_str: str, name: str):
        ...
        item = [matches.group(1) if matches.group(1) else float('nan'),
    ...
    def _transform(self, data_holder: DataHolderProtocol) -> None:
        for idx, row in data_holder.data.iterrows():
            ...
            if not lmqnt_str or lmqnt_str.isspace():
                data_holder.data.loc[idx, 'LMQNT_VAL'] = float('nan')
                continue
            ...
            if lmqnt_unit == '':
                data_holder.data.loc[idx, 'LMQNT_VAL'] = lmqnt if lmqnt is not None else float('nan')
