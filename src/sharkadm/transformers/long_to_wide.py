...
class LongToWide(Transformer):
    ...
    def _transform(self, data_holder: DataHolderProtocol) -> None:
        ...
        meta_columns = [col for col in data_holder.data.columns if col not in not_meta_columns]

        import time
        t0 = time.time()
        merge_key = 'shark_sample_id'
        merge_key = ['visit_date', 'sample_time', 'sample_depth_m']
        unique_keys = sorted(set(data_holder.data[merge_key]))
        ...
        for key, key_df in data_holder.data.groupby(merge_key):
            ...
            for par in unique_pars:
                ...
                if not len(row):
                    ...
                    par_data.extend([row['value'].values[0], row['quality_flag'].values[0]])
        ...
        for par in unique_pars:
            ...
            if par in data_holder.not_mapped_columns:
                ...
                #if unit:
                #    par = f'{par}.{unit}'
            # elif par not in data_holder.header_mapper.external_parameters:
            #     print(f'==== {par=}')
            #     par = f'{par}.{unit}'
            par_columns.append(par)
            par_columns.append(qpar)

        data_holder.data = pd.DataFrame(data=data_list, columns=meta_columns + par_columns)
        print(time.time() - t0)
