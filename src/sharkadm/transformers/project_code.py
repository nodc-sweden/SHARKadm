...
class AddSwedishProjectName(Transformer):
    ...
    #     if 'project' in data_holder.data.columns:
    ...
    #     code = row[col]
    #     info = self._loaded_code_info.setdefault(code, self._codes.get_info('project', code))
    #     if not info:
    ...
    def _get(self, row, col):
        ...
        for code in codes:
            ...
            if not info:
                adm_logger.log_transformation(f'Could not find information for {col}: {code}')
                continue
            names.append(info['swedish_name'])
        return ', '.join(names)
    ...
    def _get(self, row, col):
        ...
        for code in codes:
            ...
            if not info:
                adm_logger.log_transformation(f'Could not find information for {col}: {code}')
                continue
            names.append(info['english_name'])
        return ', '.join(names)
